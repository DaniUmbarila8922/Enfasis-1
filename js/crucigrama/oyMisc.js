...
oyCrosswordFooter.prototype.clockUpdate = function(){	
	...
	if (this.puzz.started){
		...
		document.getElementById("oygFooterClock").innerHTML = "&nbsp;Tiempo: <b>" + pad(min) + "</b>:<b>" + pad(sec) +"</b>";
	}
} 
...
oyCrosswordFooter.prototype.update = function(){	
	...
	if (!this.puzz.started){
		buf += "El Juego a\xFAn no ha comenzado!";
	} else {
		buf += "Marcador: <b>" + this.puzz.menu.score + "</b> puntos"; 		
...
function oygBind(puzz){
	...
	if (!isSupported){ 
		oygError = "No se admite este navegador.";
	} else {
		var div = document.getElementById("oygContext");
		if (div == null){
			oygError = "Archivo plantilla incorrecto.";
		} else {			
			var frame = document.getElementById("oygPuzzle");
			if (frame == null){
				oygError = "No se pudo cargar el archivo puzzle.";
			} else {
				puzz.init();
				puzz.render();
