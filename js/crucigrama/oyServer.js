...
oyServer.prototype.submitScoreAjaxAnywhere = function (ep, target, url, matches){
	...
	cp.onComplete = function(data){		 
		...
		oThis.footer.stateOk("Puntuaci\xF3n enviada!");		
	} 
		
	cp.onTimeout = function(){ 	
		oThis.footer.stateError("Tiempo de espera para que el servidor responda!");
		alert("Error al enviar el puntaje. El servidor no responde!");	 	 
	} 
	 
	cp.onError = function(msg){ 
		oThis.footer.stateError("Error al enviar el puntaje!");
		alert("Error al enviar la puntuaci\xF3n. Servidor respondi\xF3 con:\n\n" + msg);	 	 
	}
